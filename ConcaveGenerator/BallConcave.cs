    ...
    public class BallConcave
    {
        ...
        private void InitDistanceMap()
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                {
                    distanceMap[i, j] = Distance(points[i], points[j]);
                }
            }
        }
            ...
            for (int j = 0; j < list.Count; j++)
            {
                if (!flags[list[j]])
                {
                    ...
                    else
                    {
                        ...
                        if (CompareAngel(points[min], t, points[current], dp) && Distance(t,points[current])<radius)
                        {
                            min = list[j];
                        }
                    }
        ...
        public int GetNextPoint_BallPivoting(int prev, int current, List<int> list, double radius)
        {
            ...
            for (int j = 0; j < list.Count; j++)
            {
                ...
                if (!CheckHasPointsInCircle(list,rightCirleCenter,radius,adjIndex))
                {
        ...
        private bool CheckHasPointsInCircle(List<int> adjPoints,Point2d center,double radius,int adjIndex)
        {
            for (int k = 0; k < adjPoints.Count; k++)
            {
                if (adjPoints[k] != adjIndex)
                {
                    ...
                    if (InCircle(points[index2], center, radius))
                        return true;
                }
            }
            return false;
        ...
        public static bool InCircle(Point2d p,Point2d center,double r)
        {
